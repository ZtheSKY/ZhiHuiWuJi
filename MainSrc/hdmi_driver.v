...
module hdmi_driver #(
    ...
    input   [11:0]      output_length,
    input   [11:0]      output_width,
...
begin
	...
	begin
        ...
        //lcd_vs2 <= ((vcnt >= V_SYNC + V_BACK + V_DISP / 2 - output_width / 2 - v_back - v_sync - 1'b1) && (vcnt <= V_SYNC + V_BACK + V_DISP / 2 - output_width / 2 - v_back - 1'b1)) ? 1'b0 : 1'b1;
		bg <= (vcnt <= V_SYNC - 1'b1) ? background : bg;
		end
end
...
always@(posedge clk) begin
	lcd_en		<=	((hcnt >= H_SYNC + H_BACK  && hcnt < H_SYNC + H_BACK + H_DISP) &&
						...
						? 1'b1 : 1'b0;
    //if((hcnt >= H_SYNC + H_BACK  && hcnt < H_SYNC + H_BACK + H_DISP) && (vcnt >= V_SYNC + V_BACK  && vcnt < V_SYNC + V_BACK + V_DISP))
      //	begin
		if((hcnt >= H_SYNC + H_BACK + H_DISP / 2 - output_length /2 && hcnt < H_SYNC + H_BACK + H_DISP / 2 + output_length /2 ) && (vcnt >= V_SYNC + V_BACK + V_DISP / 2 - output_width / 2 && vcnt < V_SYNC + V_BACK + V_DISP / 2 + output_width / 2))
		//begin
			//if (mode == 1) begin
			//	r_lcd_rgb <= ~bg;
			//end else begin
...
//	Standard FIFO Request Port
 always@(posedge clk) begin
	lcd_request	<=	((hcnt >= H_SYNC + H_BACK + H_DISP / 2 - output_length /2 - H_AHEAD && hcnt < H_SYNC + H_BACK + H_DISP / 2 + output_length /2  - H_AHEAD) && (vcnt >= V_SYNC + V_BACK + V_DISP / 2 - output_width / 2 && vcnt < V_SYNC + V_BACK + V_DISP / 2 + output_width / 2)) ? 1'b1 : 1'b0;
